...
GROUP BY album_name, songs.id, albums.id
```

8. List all albums along with how many songs each album has

```sql
SELECT album_name, COUNT(songs.track_number)
FROM songs
WHERE count(songs.track_number) > 10
JOIN albums ON albums.id = album_id GROUP BY album_name;
```

9. Enhance previous query to only include albums that have more than 10 songs

```sql
SELECT albums.album_name, count(songs.track_number)
from songs JOIN albums on albums.id = songs.album_id
group by album_name
having count(songs.track_number) > 10;
```
...
```sql
select album_name, AVG(songs.rating) from songs
JOIN albums on albums.id = songs.album_id
group by album_name
HAVING songs.rating > AVG(songs.rating);
```
